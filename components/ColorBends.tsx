...
const ColorBends: React.FC<ColorBendsProps> = ({
  ...
  useEffect(() => {
    ...
    if (ResizeObserverCtor) {
      const ro = new ResizeObserverCtor(handleResize);
  ...
  useEffect(() => {
    ...
    const toVec3 = (hex: string) => {
      ...
      const isShort = h.length === 3 && /^[0-9a-fA-F]{3}$/.test(h);
      const isLong = h.length === 6 && /^[0-9a-fA-F]{6}$/.test(h);
      const normalized = isShort ? `${h[0]}${h[0]}${h[1]}${h[1]}${h[2]}${h[2]}` : h;
      if (!isShort && !isLong) return new THREE.Vector3(0, 0, 0);
